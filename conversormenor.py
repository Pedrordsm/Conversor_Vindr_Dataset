...
class DICOMToYOLO:
    def __init__(self, annotations_train, annotations_test, 
        ...
        self.sample_ratio = sample_ratio  # Proporção de amostragem
    ...
    def load_csvs(self):
        ...
        # Amostrar 5% dos dados
        print(f"Total de imagens de treino antes da amostragem: {len(img_train)}")
        print(f"Total de imagens de teste antes da amostragem: {len(img_test)}")
        
        # Amostrar image_labels
        img_train_sampled = img_train.sample(frac=self.sample_ratio, random_state=42)
        img_test_sampled = img_test.sample(frac=self.sample_ratio, random_state=42)
        
        # Filtrar annotations baseado nas imagens amostradas
        ann_train_sampled = ann_train[ann_train['image_id'].isin(img_train_sampled['image_id'])]
        ann_test_sampled = ann_test[ann_test['image_id'].isin(img_test_sampled['image_id'])]
        
        print(f"Total de imagens de treino após amostragem: {len(img_train_sampled)}")
        print(f"Total de imagens de teste após amostragem: {len(img_test_sampled)}")
        ...
        return ann_train_sampled, ann_test_sampled, img_train_sampled, img_test_sampled
...
"""
    ...
    def run(self):
        ...
        print(f"Usando {self.sample_ratio*100}% dos dados")
